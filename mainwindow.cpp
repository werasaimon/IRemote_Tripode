...
void MainWindow::handleConnectAndSendCommand()
{
    if(mSerialPort)
    {
      if(ui->comboBox_SerialPorts->currentText().isEmpty()) return;
     ...
     // mSerialPort->setFlowControl(QSerialPort::NoFlowControl);
      ...
      mSerialPort->setParity(QSerialPort::NoParity);
      mSerialPort->setStopBits(QSerialPort::OneStop);
      mSerialPort->setFlowControl(QSerialPort::HardwareControl); // Используйте аппаратное управление потоком
      ...
      if(mSerialPort->open(QIODevice::ReadWrite))
      {
          ...
          ui->plainTextEdit_Output->appendPlainText(QString("Port open at : ") + mSerialPort->portName());

      }
      else
      {
          qDebug() << "Failed to open tty port " << mSerialPort->portName();
          ui->plainTextEdit_Output->appendPlainText(QString("Failed to open tty port : ") + mSerialPort->portName());
      }
...
//                d = (std::abs(d) >= 50) ? d : 0;
          ...
          //           a =  0;//(SDL_JoystickGetAxis(joystick, 0) / 32768.f) * size _diff;
          //           b = -(SDL_JoystickGetAxis(joystick, 5) / 32768.f) * size_diff;
          //           c =  (SDL_JoystickGetAxis(joystick, 0) / 32768.f) * size_diff;
          //           d = -(SDL_JoystickGetAxis(joystick, 1) / 32768.f) * size_diff;
          //           e =  0;//SDL_JoystickGetButton(joystick, 6);
          //           f =  0;//SDL_JoystickGetButton(joystick, 7);
          ...
          buttonState |= (button_joystick_2 << 4);  // Режим управления каждим акутатором отдельно . бит 5
          buttonState |= (button_joystick_3 << 5); // Режим управления каждим акутатором отдельно по скорости
...
//      a = (ui->dial_angle_yaw->value() / 1000.f) * 100.f; // range -15° - 15° to Yaw
//      b = (ui->dial_angle_pitch->value() / 1000.f) * 100.f; // range -15° - 15° to Pitch
//      c = (ui->dial_angle_roll->value() / 1000.f) * 100.f; // range -15° - 15° to Roll
//      d = ui->horizontalSlider_HighPosition->value();
//      e = 0;
//      f = 0;


//      bool is_MaxSpeed = ui->checkBox_maxSpeed->isChecked();

//      buttonState = false;
//      buttonState |= (false << 0);
//      buttonState |= (false << 1); //
//      buttonState |= (true  << 2); //
//      buttonState |= (is_MaxSpeed << 3); //
//      buttonState |= (true << 4); //
//      buttonState |= (mIsShot << 5); //
//      buttonState |= (true  << 6); //
//      buttonState |= (mISFuseBlown << 7);
...
//    bool button8  = (buttonState >> 8) & 1;
//    bool button9  = (buttonState >> 9) & 1;
//    bool button10 = (buttonState >> 10) & 1;

//    printf( " button8:%d   button9:%d  button10:%d \n" , button8 , button9 , button10 );

//    qDebug() << text_time_step;
//;
...
void MainWindow::on_pushButton_SendIP_clicked()
{
    ...
    QString ipAddresStr = ui->lineEdit_IP->text();
    mSocketUDP->writeDatagram( (char*)&DataCommunication , sizeof(InterfaceCommunication) , QHostAddress(ipAddresStr) , m_Port);
}
...
void MainWindow::on_comboBox_mode_currentTextChanged(const QString &arg)
{
    ...
    if( arg == "InvidualControls-Position" ||
        arg == "InvidualControls-Velocity" ||
